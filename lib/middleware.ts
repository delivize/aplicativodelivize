...
export async function updateSession(request: NextRequest) {
  ...
  if (
    ...
    console.error(
      "[v0] Erro: Variáveis de ambiente do Supabase não configuradas corretamente!"
    );
  ...
  try {
    ...
    console.error(
      "[v0] Erro: NEXT_PUBLIC_SUPABASE_URL não é uma URL válida:",
      supabaseUrl
    );
  ...
  const isPreviewDomain =
    hostname.includes("vusercontent.net") || hostname.includes("localhost");
